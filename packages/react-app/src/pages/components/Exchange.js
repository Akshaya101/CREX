...
const Exchange = ({ pools }) => {
    ...
    const hasEnoughBalace = fromValueBigNumber.lte(fromTokenBalance ?? parseUnits("0"))
    ...
    const canSwap = !approvedNeeded && !isSwapping && fromValueIsGreaterThan0 && hasEnoughBalace
    ...
    */
    ...
    return (
        <div className='flex flex-col w-full items-center'>
            <div className='mb-8'>
                {console.log(fromValue.length)}
                <AmountIn
                    value={(fromValue == -1 || fromValue.length < 1) ? '' : fromValue}
                    onChange={onFromValueChange}
                    currencyValue={fromToken}
                    onSelect={onFromTokenChange}
                    currencies={availableTokens}
                    isSwapping={isSwapping && hasEnoughBalace}
                />
